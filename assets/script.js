...
const createPost = (postData) => {
  const { game_id, version, date, queueType, playerName, legion, buildPerWave, 
    mercenariesReceivedPerWave, leaksPerWave, categories } = postData;
  ...
  row.append(Object.assign(document.createElement('p'),
    {className: "leakpercent", innerHTML:`Total Average Leak: <span>${totalAverageLeak}%</span>`}));
  ...
  for (let waveNumber = 0; waveNumber < buildPerWave.length; waveNumber++) {
    ...
    for (unit of buildPerWave[waveNumber]) {
    ...
    {className: "leakpercent", innerHTML:`Wave ${waveNumber + 1}: <span>${totalAverageLeak}%</span>`}));
  

    sends = document.createElement("div");
    sends.className = "sends";
    sends.append(Object.assign(document.createElement('p'),{innerHTML:"Sends Received:"}));
    for (send of mercenariesReceivedPerWave[waveNumber]) {
      sendImage = get_unit_image(send)
      sends.append(Object.assign(document.createElement('img'),{alt: send, src: sendImage}));
    }
    column.append(sends);

    leaks = document.createElement("div");
    leaks.className = "leaks";
    leaks.append(Object.assign(document.createElement('p'),{innerHTML:"Leaks:"}));
    for (leak of leaksPerWave[waveNumber]) {
      leakImage = get_unit_image(leak)
      leaks.append(Object.assign(document.createElement('img'),{alt: leak, src: leakImage}));
    }
    column.append(leaks);
...
function checkFilter(post) {
  if (currentFilters['categories'].size === 0) { return true; }

  for (filterUnit of currentFilters.categories) {
    if (!post.categories.includes(filterUnit)) { return false; }
  }
  return true;
...
function calculateLeakPercentage (input) {
  ...
  for (let waveNumber = 0; waveNumber < input.length; waveNumber++) {
    waveLeakValue = 0
    for (leakedUnit of input[waveNumber]) {
